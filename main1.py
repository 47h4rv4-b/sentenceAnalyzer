...
def get_fog_index(text):
    ...
    if percentage_complex_words is not None:
...
def calculate_metrics(text):
    ...
    return {'avg_no_words': avg_no_words, 
            'complex_word_count': complex_word_count, 
            'word_count': word_count, 
            'syllable_word': syllable_word, 
            'personal_pronoun': personal_pronoun_count, 
...
articles['metrics'] = articles['clean_text'].apply(calculate_metrics)

articles['avg_no_words'] = articles['metrics'].apply(lambda x: x.get('avg_no_words', 0))
articles['complex_word_count'] = articles['metrics'].apply(lambda x: x.get('complex_word_count', 0))
articles['word_count'] = articles['metrics'].apply(lambda x: x.get('word_count', 0))
articles['syllable_word'] = articles['metrics'].apply(lambda x: x.get('syllable_word', 0))
articles['personal_pronoun'] = articles['metrics'].apply(lambda x: x.get('personal_pronoun', 0))
articles['avg_word_length'] = articles['metrics'].apply(lambda x: x.get('avg_word_length', 0))

articles = articles.drop(['clean_text', 'metrics'], axis=1)
articles.to_excel('articles_sentiment_scores1.xlsx', index=False)
